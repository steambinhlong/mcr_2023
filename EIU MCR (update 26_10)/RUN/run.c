...
/* Main program                                                         */
/************************************************************************/
signed char sensorPos, straightLine;	// sensorState & straightLine flag
unsigned int RunTimer;	// maximum time run of RC
signed int line;	
//////////////////////////////////////////////////////// forward line
int speedRunForward;		// straight speed 
int speedRun0; int angle0;		// speed0 depend on error_angle0
int speedRun1; int angle1;		// speed1 depend on error_angle1
int speedRun2; int angle2;		// speed2 depend on error_angle2
int speedRun3; int angle3;		// speed3 depend on error_angle3
int speedRun4; int angle4;		// speed4 depend on error_angle4
int speedRun5; int angle5;		// speed5 depend on error_angle5
int speedRun6; int angle6;		// speed6 depend on error_angle6
int speedRun7; int angle7;		// speed7 depend on error_angle7
int speedRun8; int angle8;		// speed8 depend on error_angle8

//////////////////////////////////////////////////////// turn90
signed int speedBrake90;		// 0 is maximum brake
unsigned int timerBrake90;		// time longer, more brake
signed int speed90;				// speed when brake at 90 degrees curve
signed int speedBoostAfter90;	// speed boost after 90 degrees curve
unsigned int timerBoostAfter90;	// time to accelerate

//////////////////////////////////////////////////////// change lanes
int angleChangeLanes;						// angle to change lane
signed int speedBrakeChangeLanes;			// speed when brake to change lane 
unsigned int timerBrakeChangeLanes;			// time brake to change lane
signed int speedChangeLanes;				// speed when recognize lane
signed int speedBoostAfterChangeLanes;		// speed after change lane
unsigned int timerBoostAfterChangeLanes;	// time to accelerate
...
void setup()
{
	handle( 0 );		// set servo angle as 0
	speed( 0, 0 );		// set motor speed as 0
	cnt1 = 0;			// reset cnt1
	cnt4 = 0;			// reset cn2
	RunTimer = 30000;	// Intial RunTimer - can change if u want
	sensorPos = 0;		// set status of IR sensors as 0b0000 0000 
	straightLine = 1;	// set flag of straight line (default) is true
	switch (get_dipsw())
	{
		case 7: // 25 %
		//////////////////////////////////////////////////////// turn90
...
void run(void)
{
	...
	while( 1 ) {
		switch( runCase ) {
			...
			break;
			case 2:
			//if( cnt1 < 3000 ) {// ch? 3s xu?t ph�t
			if( cnt1 < 1 ) {// xu?t ph�t ngay khi b?m n�t SW4
				handle(0);
				speed(0,0);
			}
			else {
...
{
	if (checkCrossLine() == 0){
		if (sensorMask(0xf8)==0xf8)//11111000
		return HALF_LEFT_LINE;
		else if (sensorMask(0x1f)==0x1f)//00011111
